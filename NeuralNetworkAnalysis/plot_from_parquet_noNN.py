...
def StackPlot_NoSig(background_array,background_list,data_array,plot_name):
    fig,ax = plt.subplots()
    hep.histplot(background_array,ax=ax,stack=True,histtype="fill",label=background_list,facecolor=TABLEAU_COLORS[:len(background_list)],edgecolor=TABLEAU_COLORS[:len(background_list)])
    hep.histplot(data_array,ax=ax,stack=False,histtype="errorbar", yerr=True,label=["Data"],marker="o",color = "k") 
    hep.cms.text("Preliminary",loc=0,fontsize=13)
    ...
    for mass in mass_array:
        for region_name in region_array:
            ...
            var_array = ["radion_pt","vis_mass","vis_mass2","radion_eta","higgs1_dr","higgs2_dr","dphi_H1","dphi_H2",
                    "dphi_H1_MET","dphi_H2_MET","dr_HH","dphi_HH","dr_H1_Rad","dr_H2_Rad","dphi_rad_MET","H1OS","H2OS","numBJet","ZMult","RecoRadion_Mass"] #,"weight","predictions"]
            
            #Variables for plotting
            dict_plot_names = {"radion_pt": "radion_pT_Mass_" + mass + "-EitherOr_Trigger","vis_mass": "LeadingHiggs_Mass_"+ mass + "-EitherOr_Trigger",
                    "vis_mass2": "SubleadingHiggs_Mass_"+ mass + "-EitherOr_Trigger", "radion_eta": "Radion_eta_Mass_" + mass + "-EitherOr_Trigger", 
                    ...
                    "ZMult": "ZMult_Mass" + mass + "-EitherOr_Trigger", 
                    "RecoRadion_Mass": "RecoRadion_Mass" + mass + "-EitherOr_Trigger"}
            data_files = ["Data_JetHT","Data_SingleMuon"]
            #data_files = ["Data_SingleMuon"]
            #signal_file = ["Signal"]
            background_list = [r"$t\bar{t}$", r"Drell-Yan+Jets", "Di-Bosons", "Single Top", "W+Jets", r"$ZZ \rightarrow 4l$"]
            #background_list = [r"Drell-Yan+Jets", "Di-Bosons", "Single Top", r"$ZZ \rightarrow 4l$"]
            ...
            for background_type in background_dict:
                for background in background_dict[background_type]:
                    #background_input_dict[background] = pd.read_parquet("miniAOD_Parquet_Files/" + background + ".parquet",engine="pyarrow")
                    background_input_dict[background] = pd.read_parquet(background + ".parquet",engine="pyarrow")

            #signal_input = pd.read_parquet("Signal_mass_" + mass + "GeV.parquet",engine="pyarrow")

            data_input_dict = {}
            for data_file in data_files:
                #data_input_dict[data_file] = pd.read_parquet("miniAOD_Parquet_Files/" + data_file + ".parquet",engine="pyarrow")
                data_input_dict[data_file] = pd.read_parquet(data_file + ".parquet",engine="pyarrow")
            ...
            hist_dict_background = {
                    ...
                    "vis_mass": hist.Hist.new.StrCat([r"$t\bar{t}$", r"Drell-Yan+Jets", "Di-Bosons", "Single Top", "W+Jets", r"$ZZ \rightarrow 4l$"],name="background").Reg(N2,0,200, label=r"Leading di-$\tau$ Mass (GeV)").Double(),
                    "vis_mass2": hist.Hist.new.StrCat([r"$t\bar{t}$", r"Drell-Yan+Jets", "Di-Bosons", "Single Top", "W+Jets", r"$ZZ \rightarrow 4l$"],name="background").Reg(N2,0,200, label=r"Subleading di-$\tau$ Mass (GeV)").Double(),
            ...
            hist_dict_data = {
                    ...
                    "vis_mass": hist.Hist.new.StrCat(["Data"],name="data").Reg(N2,0,200, label=r"Leading di-$\tau$ Mass (GeV)").Double(),
                    "vis_mass2": hist.Hist.new.StrCat(["Data"],name="data").Reg(N2,0,200, label=r"Subleading di-$\tau$ Mass (GeV)").Double(),
            ...
            for hist_name in var_array:
                ...
                for background_type in background_list:
                    for background in background_dict[background_type]:
                        ...
                        #print("Sum value")
                        if (region_name == "All"):
                            print("Sum of weight for " + background +":")
                            print(np.sum(background_input_dict[background]["weight"].to_numpy()))
                ...
                #Fill in signal
                #hist_dict_signal[hist_name].fill("Signal",signal_input[hist_name].to_numpy(),weight=signal_input["weight"].to_numpy()*5)

                #Fill in the data
                for data_file in data_files:
                    hist_dict_data[hist_name].fill("Data",data_input_dict[data_file][hist_name].to_numpy())
                    if (region_name == "All"):
                        print("Sum of data entries for data " + data_file)
                        print(len(data_input_dict[data_file][hist_name].to_numpy()))
                    #print(data_input_dict[data_file][hist_name].to_numpy())
                    #print("Data array length:")
                    #print(len(data_input_dict[data_file][hist_name].to_numpy()))
                if (region_name == "All"):
                    print("Data histogram sum: %d"%hist_dict_data[hist_name].sum())

				#print("Data histogram sum: %d"%hist_dict_data.sum())
				#print("Data histogram sum: %d"%hist_dict_data.sum())
                ...
                for background in background_list:
                    ...
                    if (region_name == "ALl"):
                        print("Sum of histogram for " + background + " background: %d"%background_stack[background].sum())
                ...
                if (region_name == "HighPurity"):
                    ...
                    StackPlot_NoSig(background_array,background_list,data_array,dict_plot_names[hist_name] + "_" + region_name + "_single_bin") 
                else:
                    #StackPlot(background_array,background_list,data_array,dict_plot_names[hist_name] + "_" + region_name) 
                    #StackPlot(signal_array,signal_list,background_array,background_list,data_array,dict_plot_names[hist_name] + "_" + region_name) 
                    StackPlot_NoSig(background_array,background_list,data_array,dict_plot_names[hist_name] + "_" + region_name) 

...
def StackPlot(signal_array,signal_list,background_array,background_list,data_array,plot_name):
    ...
    hep.histplot(signal_array,ax=ax,stack=True,histtype="step",label=signal_list,edgecolor=TABLEAU_COLORS[len(background_list):len(background_list)+2],linewidth=2.95)
...
#Produce stacked histogram corresponding to control + signal regions
#def StackedControlSignalRegion(signal_input,background_input_dict,background_dict,data_input_dict,data_files,background_list,region_name,hist_dict_background,hist_dict_signal,hist_dict_data,plot_name):
#    if (region_name == "ZCR"):
...
#                background_input_dict[background] = background_input_dict[background][np.bitwise_or(background_input_dict[background].ZMult >= 1, background_input_dict[background].numBJet < 1)]
#        for data in data_files:
#            data_input[data] = data_input[data][np.bitwise_and(data_input[data].ZMult >= 1,data_input[data].numBJet < 1)]
#    if (region_name == "TCR"):
#        signal_input = signal_input[np.bitwise_and(signal_input.numBJet >= 1, signal_input.ZMult < 1)]
#        for (background_type in background_dict):
#            for background in background_dict[background_type]: 
#                background_input_dict[background] = background_input_dict[background][np.bitiwise_and(background_input_dict[background].numBJet >= 1, background_input_dict[background].ZMult < 1)]
#        for data in data_files:
#            data_input[data] = data_input[data][np.bitwise_and(data_input[data].numBJet >= 1, data_input[data].ZMult < 1)]
...
#        for data in data_files:
#            data_input[data] = data_input[data][np.bitwise_and(data_input[data].ZMult < 1,data_input[data].numBJet < 1)]
#            data_input[data] = data_input[data][np.bitwise_or(data_input[data].H1OS != 1,data_input[data].H2OS != 0)]
...
if __name__ == "__main__":
    ...
    region_array = ["All","ZCR","TCR","FakeCR","LowPurity","HighPurity"] 
    ...
    for region_name in region_array:
        ...
        var_array = ["radion_pt","vis_mass","vis_mass2","radion_eta","higgs1_dr","higgs2_dr","dphi_H1","dphi_H2",
                "dphi_H1_MET","dphi_H2_MET","dr_HH","dphi_HH","dr_H1_Rad","dr_H2_Rad","dphi_rad_MET","H1OS","H2OS","numBJet","predictions","ZMult","RecoRadion_Mass"] #,"weight","predictions"]
        
        #Variables for plotting
        dict_plot_names = {"radion_pt": "radion_pT_AllMass_EitherOr_Trigger","vis_mass": "LeadingHiggs_AllMass_EitherOr_Trigger",
                "vis_mass2": "LeadingHiggs_AllMass_EitherOr_Trigger", "radion_eta": "Radion_eta_AllMass_EitherOr_Trigger", 
                "higgs1_dr": "leading_diTau_DeltaR_AllMass_EitherOr_Trigger", "higgs2_dr": "subleading_diTau_DeltaR_AllMass_EitherOr_Trigger",
                "dphi_H1": "LeadingdiTau_DeltaPhi_AllMass_EitherOr_Trigger", "dphi_H2": "SubleadingdiTau_DeltaPhi_AllMass_EitherOr_Trigger",
                "dphi_H1_MET": "LeadingHiggs_MET_DeltaPhi_AllMass_EitherOr_Trigger", "dphi_H2_MET": "SubleadingHiggs_MET_DeltaPhi_AllMass_EitherOr_Trigger",
                "dr_HH": "Higgs_DeltaR_AllMass_EitherOr_Trigger", "dphi_HH": "HiggsDeltaPhi_AllMass_EitherOr_Trigger" ,"dr_H1_Rad": "LeadingHiggs_Radion_DeltaR_AllMass_EitherOr_Trigger",
                "dr_H2_Rad": "SubLeadingHiggs_Radion_DeltaR_AllMass_EitherOr_Trigger", "dphi_rad_MET": "Radion_MET_DeltaPhi_AllMass_EitherOr_Trigger",
                "H1OS":"LeadingHiggs_Charge_AllMass_EitherOr_Trigger","H2OS": "SubleadingHiggs_Charge_AllMass_EitherOr_Trigger", "numBJet": "BJetMult_AllAllMass_EitherOr_Trigger", 
                "predictions": "NeuralNetwork_Prediction_AllMass_EitherOr_Trigger", "ZMult": "ZMult_AllMass_EitherOr_Trigger", 
                "RecoRadion_Mass": "RecoRadion_AllMass_EitherOr_Trigger"}
        ...
        signal_list = [r"MC Sample $m_\phi$ = 1 TeV",r"MC Sample $m_\phi$ = 2 TeV",r"MC Sample $m_\phi$ = 3 TeV"]
        ...
        signal_input_dict = {}
        for mass in mass_array:
            signal_input_dict[mass] = pd.read_parquet("Signal_mass_" + mass + "GeVwith_predictions.parquet",engine="pyarrow")
        #signal_input = pd.read_parquet("Signal_mass_" + mass + "GeVwith_predictions.parquet",engine="pyarrow")
        ...
        hist_dict_signal = {
                "radion_pt": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,0,500, label=r"Radion $p_T$ (GeV)").Double(), 
                "vis_mass": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N2,0,120, label=r"Leading di-$\tau$ Mass (GeV)").Double(),
                "vis_mass2": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N2,0,120, label=r"Subleading di-$\tau$ Mass (GeV)").Double(),
                "radion_eta": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,-5,5, label = r"Radion $\eta$").Double(),
                "higgs1_dr": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,0,5, label = r"Leading di-$\tau$ $\Delta$R").Double(),
                "higgs2_dr": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,0,5, label = r"Subleading di-$\tau$ $\Delta$R").Double(),
                "dphi_H1": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,-pi,pi, label = r"Subleading di-$\tau$ $\Delta \phi$").Double(),
                "dphi_H2": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,-pi,pi, label = r"Leading di-$\tau$ $\Delta \phi$").Double(),
                "dphi_H1_MET": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,-pi,pi, label = r"Leading Higgs MET $\Delta \phi$").Double(),
                "dphi_H2_MET": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,-pi,pi, label = r"Subleading Higgs MET $\Delta \phi$").Double() ,
                "dr_HH": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,0,5, label = r"Higgs $\Delta$R").Double(),
                "dphi_HH": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,-pi,pi, label = r"Higgs $\Delta \phi$").Double(),
                "dr_H1_Rad": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,0,5, label = r"Leading Higgs Radion $\Delta$R").Double(),
                "dr_H2_Rad": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,0,5, label = r"Subleading Higgs Radion $\Delta$R").Double(),
                "dphi_rad_MET": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,-pi,pi, label = r"Radion MET $\Delta \phi$").Double(),
                "H1OS": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name = "signal").Reg(8,-4,4,label = r"Leading Higgs Electric Charge").Double(),
                "H2OS": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name = "signal").Reg(8,-4,4,label = r"Subleading Higgs Electric Charge").Double(),
                "numBJet": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(6,0,6, label = r"B Jet Multiplicity").Double(),
                "ZMult": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(6,0,6, label = r"Z Boson Multiplicity").Double(),
                "predictions": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N1,0,1.0, label = r"Neural Network Model Output").Double(),
                "RecoRadion_Mass": hist.Hist.new.StrCat([r"$m_\phi$ = 1 TeV", r"$m_\phi$ = 2 TeV", r"$m_\phi$ = 3 TeV"],name="signal").Reg(N_fourMass,0,3000, label = r"Reconstructed Radion Mass (GeV)").Double(),
            }

        if (region_name == "HighPurity" or region_name == "All"): #Blind the data
            reco_mass = 0.01
        else:
            reco_mass = 3000
        ...
        if (region_name == "ZCR"):
            for mass in mass_array:
                signal_input_dict[mass] = signal_input_dict[mass][np.bitwise_and(signal_input_dict[mass].ZMult >= 1,signal_input_dict[mass].numBJet < 1)]
            for background_type in background_dict:
                for background in background_dict[background_type]: 
                    background_input_dict[background] = background_input_dict[background][np.bitwise_or(background_input_dict[background].ZMult >= 1, background_input_dict[background].numBJet < 1)]
            for data in data_files:
        ...
        if (region_name == "TCR"):
            for mass in mass_array:
                signal_input_dict[mass] = signal_input_dict[mass][np.bitwise_and(signal_input_dict[mass].numBJet >= 1, signal_input_dict[mass].ZMult < 1)]
            for background_type in background_dict:
                for background in background_dict[background_type]: 
                    background_input_dict[background] = background_input_dict[background][np.bitwise_and(background_input_dict[background].numBJet >= 1, background_input_dict[background].ZMult < 1)]
            for data in data_files:
                data_input_dict[data] = data_input_dict[data][np.bitwise_and(data_input_dict[data].numBJet >= 1, data_input_dict[data].ZMult < 1)]
        if (region_name == "FakeCR"):
            for mass in mass_array:
                signal_input_dict[mass] = signal_input_dict[mass][np.bitwise_and(signal_input_dict[mass].ZMult < 1,signal_input_dict[mass].numBJet < 1)]
                signal_input_dict[mass] = signal_input_dict[mass][np.bitwise_or(signal_input_dict[mass].H1OS != 0,signal_input_dict[mass].H2OS != 0)]
            for background_type in background_dict:
                for background in background_dict[background_type]: 
                    background_input_dict[background] = background_input_dict[background][np.bitwise_and(background_input_dict[background].ZMult < 1,background_input_dict[background].numBJet < 1)]
                    background_input_dict[background] = background_input_dict[background][np.bitwise_or(background_input_dict[background].H1OS != 0,background_input_dict[background].H2OS != 0)]
        ...
        if (region_name == "LowPurity"):
            for mass in mass_array:
                signal_input_dict[mass] = signal_input_dict[mass][np.bitwise_and(signal_input_dict[mass].ZMult < 1,signal_input_dict[mass].numBJet < 1)]
                signal_input_dict[mass] = signal_input_dict[mass][np.bitwise_and(signal_input_dict[mass].H1OS == 0,signal_input_dict[mass].H2OS == 0)]
                signal_input_dict[mass] = signal_input_dict[mass][signal_input_dict[mass].predictions < 0.7]
            for background_type in background_dict:
                for background in background_dict[background_type]: 
                    background_input_dict[background] = background_input_dict[background][np.bitwise_and(background_input_dict[background].ZMult < 1,background_input_dict[background].numBJet < 1)]
        ...
        for hist_name in var_array:
            ...
            #Fill in signal
            for mass in mass_array:
                hist_dict_signal[hist_name].fill(r"$m_\phi$ = %s TeV"%mass[0],signal_input_dict[mass][hist_name].to_numpy(),weight=signal_input_dict[mass]["weight"].to_numpy()*5)
            ...
            signal_array = []
            #for mass in mass_array: 
            #    signal_array.append(signal_stack[r"$m_\phi$ = %s TeV"%mass[0]])
            signal_array.append(signal_stack[r"$m_\phi$ = 1 TeV"])
            signal_array.append(signal_stack[r"$m_\phi$ = 2 TeV"])
            signal_array.append(signal_stack[r"$m_\phi$ = 3 TeV"])

...
if [ "\$1" == "" ]; then
	#export COFFEA_IMAGE=coffeateam/coffea-base-almalinux8:0.7.26-py3.10
...
EXTERNAL_BIND=\${PWD} singularity exec -B \${PWD}:/srv -B /etc/condor -B /scratch -B /hdfs -B /nfs_scratch -B /afs --pwd /srv \\

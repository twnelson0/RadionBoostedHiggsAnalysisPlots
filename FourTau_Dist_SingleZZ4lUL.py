...
class FourTauPlotting(processor.ProcessorABC):
	...
	def process(self, events):
		...
		print(ak.num(events.boostedTauPt))
		print(ak.num(events.boostedTauEnergy))
		print(ak.num(events.boostedTauPx))
		print(ak.num(events.boostedTauPy))
		print(ak.num(events.boostedTauPz))
		print(ak.num(events.boostedTauMass))
		print(ak.num(events.boostedTauEta))
		print(ak.num(events.boostedTauPhi))
		print(ak.num(events.boostedTauCharge))
		print(ak.num(events.boostedTauByIsolationMVArun2v1DBoldDMwLTrawNew))
		print(ak.num(events.boostedTaupfTausDiscriminationByDecayModeFinding))
		...
		tau = ak.zip( 
			{
				...
				"iso": events.boostedTaupfTausDiscriminationByDecayModeFinding, #WHY IS THIS EMPTY THE SINGLE FILE???
		...
		#	)
			...
			event_level["event_weight"] = event_level.event_weight**0 #GenTau_Num #Apply weightings based on Gen Taus
			...
			if (self.PU_bool): #Apply PU reweighting scheme (Single file has no PU info)
	...
	#	event_level["HT"] = ak.sum(Jet_HT.Pt, axis = 1, keepdims=False)
		...
		if (not(self.isData)):	#MC trigger logic
			...
			#		Gen_Info_39 = Gen_Info_fail[np.bitwise_and(event_level_fail.jet_trigger,bit_mask([39])) == bit_mask([39])]
				...
				if (np.pi == np.exp(1)): #Turn off offline selections
	...
	#		Gen_Info = Gen_Info[ak.num(tau) >= 4] 
		...
		if (ak.num(event_level.pfMET,axis=0) > 0):
		...
		event_level["nBJets"] = ak.ones_like(event_level.pfMET) #Again no jets so no B-Jet Multiplcity
...
if __name__ == "__main__":
	...
	for mass in mass_str_arr:
		...
		file_dict = { #Reduced files to run over
			...
			"ZZ4l": ["/hdfs/store/user/abdollah/skimmed_ZZ_oneFile_UL.root"],
		...
		for key_name, file_name in file_dict.items(): 
			...
			if (key_name != "Data_JetHT" and key_name != "Data_SingleMuon"):
				...
				print(file_name)
				numEvents_Dict[key_name] = tempFile['hEvents'].member('fEntries')/2 #Needed for single UL file for some reason
		...
		for trigger_name, trigger_pair in trigger_dict.items(): #Run over all triggers/combinations of interest
			...
			#fourtau_out = iterative_runner(file_dict, treename="ggNtuplizer/EventTree", processor_instance=FourTauPlotting(trigger_bit=trigger_pair[0], or_trigger=trigger_pair[1],PUWeights = PUWeight, PU_weight_bool =False, signal_mass = mass)) #I don't know why the single UL file structure is different
			#Produce cutflow csv table
			for file in file_dict.keys():
				cutflow_table_array.append(fourtau_out[file]["cutflow_dict"])
			with open("Cutflow_Table_Mass_" + mass[0] + "TeV_fromMiniAOD.csv", mode = "w", newline = '') as cutflow_file:	
